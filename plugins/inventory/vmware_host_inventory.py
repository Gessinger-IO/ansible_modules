...
# Copyright: (c) 2019, René Gessigner <rene.gessinger@concat.de>
#
...
DOCUMENTATION = '''
    ...
    author:
      - René Gessinger
    description:
        - Get esxi hosts as inventory hosts from VMware environment.
        ...
        - The inventory_hostname is always the 'Name' of the host.
    ...
    options:
        ...
        port:
            description: Port number used to connect to vCenter.
            default: 443
            env:
              - name: VMWARE_PORT
        validate_certs:
        ...
        properties:
            description:
            - Specify the list of VMware schema properties associated with the host.
        ...
        group_by_cluster:
            description:
            - Group hosts by cluster if available.
            - Requires 'vSphere Automation SDK' library to be installed on the given controller machine.
            - Please refer following URLs for installation steps
            - 'https://code.vmware.com/web/sdk/65/vsphere-automation-python'
            default: False
        ...
        group_by_maintenance:
            description:
            - Group hosts by maintenance mode.
            - Requires 'vSphere Automation SDK' library to be installed on the given controller machine.
            - Please refer following URLs for installation steps
            - 'https://code.vmware.com/web/sdk/65/vsphere-automation-python'
            default: True
...
# Gather minimum set of properties for VMware ESXi
    plugin: vmware_host_inventory
    strict: False
    hostname: 10.65.223.31
    username: administrator@vsphere.local
...
class BaseVMwareInventory:
    def __init__(self, hostname, username, password, port, validate_certs, with_tags, group_by_cluster, group_by_datacenter, group_by_maintenance):
        ...
        self.group_by_datacenter = group_by_datacenter
        self.group_by_maintenance = group_by_maintenance
    ...
    def _login(self):
        ...
        try:
            ...
            raise AnsibleParserError("Unable to log on to vCenter at %s:%s as %s: %s" % (self.hostname, self.port, self.username, e.msg))
        except vim.fault.NoPermission as e:
            raise AnsibleParserError("User %s does not have required permission"
                                     " to log on to vCenter at %s:%s : %s" % (self.username, self.hostname, self.port, e.msg))
        except (requests.ConnectionError, ssl.SSLError) as e:
            raise AnsibleParserError("Unable to connect to vCenter at %s on TCP/%s: %s" % (self.hostname, self.port, e))
            ...
            raise AnsibleParserError("Unknown error while connecting to vCenter at %s:%s : %s" % (self.hostname, self.port, e))
    ...
    def _get_managed_objects_properties(self, vim_type, properties=None):
        """
        Look up a Managed Object Reference in vCenter
    ...
    @staticmethod
    def _get_object_prop(host, attributes):
...
class InventoryModule(BaseInventoryPlugin, Cacheable):
    ...
    def verify_file(self, path):
        ...
        if super(InventoryModule, self).verify_file(path):
            if path.endswith(('vmware.yaml', 'vmware.yml', 'vmware_host_inventory.yaml', 'vmware_host_inventory.yml')):
                valid = True
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        self.pyv = BaseVMwareInventory(
            ...
            group_by_cluster=self.get_option('group_by_cluster'),
            group_by_datacenter=self.get_option('group_by_datacenter'),
            group_by_maintenance=self.get_option('group_by_maintenance')
        )
    ...
    def _populate_from_source(self, source_data, using_current_cache):
        ...
        hostvars = {}
        objects = self.pyv._get_managed_objects_properties(vim_type=vim.HostSystem,
                                                           properties=['name'])
        ...
        if self.pyv.with_tags:
            ...
            for tag in tags:
                ...
                tags_info[tag_obj.id] = "tag_" + tag_obj.name
                if ("tag_" + tag_obj.name) not in cacheable_results:
                    cacheable_results[("tag_" + tag_obj.name)] = {'hosts': []}
                    self.inventory.add_group(("tag_" + tag_obj.name))

        for host_obj in objects:
            for host_obj_property in host_obj.propSet:
                current_host = host_obj_property.val

                if current_host not in hostvars:
                    hostvars[current_host] = {}
                    self.inventory.add_host(current_host)

                    host_ip = host_obj.obj.summary.config.name
                    if host_ip:
                        self.inventory.set_variable(current_host, 'ansible_host', host_ip)

                    self._populate_host_properties(host_obj, current_host)
                    ...
                    if HAS_VSPHERE and self.pyv.with_tags:
                        ...
                        attached_tags = tag_association.list_attached_tags(host_dynamic_id)
                        ...
                        for tag_id in attached_tags:
                            ...
                            tag_model = self.pyv.rest_content.tagging.Tag.get(tag_id)
                            tag_list.append(tag_model.name)
                        self.inventory.set_variable(current_host, 'attached_tags', tag_list)

                    # Based on connection state of host
                    host_state = str(host_obj.obj.summary.runtime.connectionState)
                    if host_state not in cacheable_results:
                        cacheable_results[host_state] = {'hosts': []}
                        self.inventory.add_group(host_state)
                    cacheable_results[host_state]['hosts'].append(current_host)
                    self.inventory.add_child(host_state, current_host)

                    # Based on maintenance state of host
                    if self.pyv.group_by_maintenance:
                        maintenance_mode = str(host_obj.obj.summary.runtime.inMaintenanceMode)
                        if maintenance_mode is "True":
                            if "Maintenance" not in cacheable_results:
                                cacheable_results["Maintenance"] = {'hosts': []}
                                self.inventory.add_group("Maintenance")
                            cacheable_results["Maintenance"]['hosts'].append(current_host)
                            self.inventory.add_child("Maintenance", current_host)

                    # Based on datacenter of host
                    if self.pyv.group_by_datacenter:
                        datacenter = "dc_" + str(self.get_datacenter_from_host(host_obj.obj))
                        if datacenter not in cacheable_results:
                            cacheable_results[datacenter] = {'hosts': []}
                            self.inventory.add_group(datacenter)
                        cacheable_results[datacenter]['hosts'].append(current_host)
                        self.inventory.add_child(datacenter, current_host)

                    # Based on cluster of host (if any)
                    if self.pyv.group_by_cluster:
                        if type(host_obj.obj.parent).__name__ == 'vim.ClusterComputeResource':
                            cluster = "cl_" + host_obj.obj.parent.name
                            if cluster not in cacheable_results:
                                cacheable_results[cluster] = {'hosts': []}
                                self.inventory.add_group(cluster)
                            cacheable_results[cluster]['hosts'].append(current_host)
                            self.inventory.add_child(cluster, current_host)
    ...
    def _populate_host_properties(self, host_obj, current_host):
        # Load ESXi properties in host_vars
